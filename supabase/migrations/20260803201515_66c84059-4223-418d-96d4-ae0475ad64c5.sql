delete from public.calendar_events where email='sid1612@gmail.com';
...
select 'sid1612@gmail.com', c.id, 'demo-'||g||'-'||i,
  ...
  (current_date + g)::timestamptz + make_interval(hours => 8 + i*2),
  (current_date + g)::timestamptz + make_interval(hours => 8 + i*2, mins => 45),
  (array[6,4,2,3,1,5,8,7,1,9])[i], (i % 3 = 0), 'google'
from generate_series(0,6) g, generate_series(1,3) i,
